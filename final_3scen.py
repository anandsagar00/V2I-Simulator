...
# Carbon emission of vehicles per second in micro-grams
carbonValue = {"car": 0.8, "bus": 1.2,
               "truck": 1.3, "rickshaw": 0.9, "bike": 0.3}
            ...
            if(self.willTurn == 1):
                ...
                if((self.x+self.currentImage.get_rect().width <= self.stop or self.crossed == 1 or (currentGreen == 0 and currentYellow == 0)) and (self.index == 0 or self.x+self.currentImage.get_rect().width < (vehicles[self.direction][self.lane][self.index-1].x - gap2) or (vehicles[self.direction][self.lane][self.index-1].turned == 1))):
                    ...
                    if self.isOn == 1:
                        ...
                        # if isOn is 0 then no carb emission , else carbon emission
                        totalCarbonEmission += (
                            carbonValue[self.vehicleClass]*self.isOn)
...
def setTime():
    global right_model, left_model, up_model, down_model, iter
    ...
    if direction == "right":
        ...
        totalVehiclesLane1 = len(
            vehicles['right'][0])+len(vehicles['right'][1])+len(vehicles['right'][2])
        totalVehiclesLane2 = len(
            vehicles['down'][0])+len(vehicles['down'][1])+len(vehicles['down'][2])
        totalVehiclesLane3 = len(
            vehicles['left'][0])+len(vehicles['left'][1])+len(vehicles['left'][2])
        totalVehiclesLane4 = len(
            vehicles['up'][0])+len(vehicles['up'][1])+len(vehicles['up'][2])
        totalVehiclesNow = totalVehiclesLane1+totalVehiclesLane2 + \
            totalVehiclesLane3+totalVehiclesLane4

        avg_waiting_time = math.ceil(totalWaitingTime/totalVehicles)

        file = open('data_prop_high.csv', 'a')
        file.write(str(totalVehiclesLane1)+','+str(totalVehiclesLane2)+',' +
                   str(totalVehiclesLane3)+','+str(totalVehiclesLane4)+','+str(totalVehiclesNow)+','+str(math.ceil(avg_waiting_time))+'\n')
        file.close()
    ...
    print("Vehicles : ", totalVehiclesLane1, totalVehiclesLane2,
          totalVehiclesLane3, totalVehiclesLane4)
    print("Debug GT1", greenTime1, " GT2 ", greenTime2,
          " GT3 ", greenTime3, "GT4 : ", greenTime4)
...
def generateVehicles():
    while(True):
        ...
        # changing the parameter of this function would affect how fast or slow the vehicles are generated
        time.sleep(0.1)
...
class Main:
    ...
    while True:
        ...
        if totalCarbonEmission < 1000:
            carbonText = font.render(
                ("Carbon Emission : "+str(round(totalCarbonEmission, 2))+"  µg"), True, black, white)
        elif totalCarbonEmission >= 1000 and totalCarbonEmission < 1000000:
            carbonText = font.render(
                ("Carbon Emission : "+str(round(totalCarbonEmission/1000, 2))+"  mg"), True, black, white)
        elif totalCarbonEmission >= 1000000 and totalCarbonEmission < 1000000000:
            carbonText = font.render(
                ("Carbon Emission : "+str(round(totalCarbonEmission/1000000, 2))+"  g"), True, black, white)
        else:
            carbonText = font.render(
                ("Carbon Emission : "+str(round(totalCarbonEmission/1000000000, 2))+"  Kg"), True, black, white)
